...
- 添加统一的五方法对比实验：固定导纳、规则自适应、模糊规则、SAC 和 PPO。
- 添加轻度/中度/重度虚拟患者矩阵、多随机种子和逐 episode 指标记录。
- 添加 CSV/JSON 统计表、成功率/误差/交互力/参数稳定性 PNG 图表和自动 Markdown 报告。
- 添加无头 MuJoCo MP4 演示视频生成器。
- 添加一键实验脚本、Phase 10 配置、项目报告、简历描述和新环境复现说明。
- 明确实验结果仅用于仿真/开发对比，不替代真实机器人和人体安全验证。

### Phase 11

- 添加 LLM 交互层（DeepSeek / OpenAI 兼容接口）：事件消息润色、结构化训练总结和患者/治疗师问答。
- 规则 Agent 保留实时事件检测；LLM 只异步增强表达，失败自动回退规则模板，不进入控制环。
- 新增 `asyncio` 后台 worker 队列，LLM 调用与 20 Hz 控制环完全解耦，API Key 仅从环境变量读取。
- 新增 `POST /api/agent/chat` 对话端点、`agent_chat` 消息流（规则/LLM/用户来源标记）和总结来源标记。
- 新增前端智能教练聊天面板（实时消息流 + 提问输入框）。
- 前端重构为单页无滚动双视图：首页任务库 + 训练页左 2/3 数据模块、右 1/3 聊天框。
- 新增迷宫导航与色块记忆任务（后端仿真 + 前端 MazeChart/ColorMemoryPanel 可视化），任务库扩展至三大类五项。
- 添加 mock HTTP 传输的 LLM 单元测试与后端集成测试（503 回退、规则消息流）。
